...
>
> **Revision note (2026-07-19)**: Finding **G3** identified that `POST /sso/login` (T026) grants access through *either* branch — `provider is not None and provider.enabled` *or* `demo.enabled` — even though contracts/sso-config-api.md already documents this endpoint as "demo-mode-only... not the production path." An enabled real provider therefore lets any caller POST an arbitrary `{ email }` and be granted a role with **no token/identity verification**, entirely bypassing the verified `/sso/authorize` → `/sso/callback` handshake (T035/T036) that FR-009/FR-010 require for real-provider access. T042b (new) removes the `provider.enabled` branch so this endpoint's only grant path is `demo_mode.enabled`. Per the constitution's Development Workflow clause ("API changes MUST preserve compatibility or include a documented migration plan"), this is called out explicitly as a **behavior change to a already-shipped endpoint (T026 is marked done)**: any caller currently relying on the undocumented provider-branch grant from `POST /sso/login` loses that path and must use `/sso/authorize` instead. T043 is updated to depend on T042b.
...
- [X] T033 [P] [US2] Add `itsdangerous` to `server/requirements.txt` (justification: hard transitive requirement of Starlette's `SessionMiddleware`, itself required by Authlib's Starlette OAuth client for `state`/`nonce` storage during the redirect round trip — see research.md)
- [X] T034 [US2] Add `session_secret_key: str = ""` to `Settings` in `server/app/config.py`, document `SESSION_SECRET_KEY` in `server/.env.example`, and register `starlette.middleware.sessions.SessionMiddleware(app, secret_key=settings.session_secret_key)` in `server/app/main.py` (depends on T033)
- [X] T035 [US2] Implement `GET /sso/authorize` in `server/app/routers/sso.py`: build the OAuth client via `build_oauth_client` (T010b) from the stored provider config and call `await client.authorize_redirect(request, provider.redirect_uri)`; return 503 if no provider is configured/enabled (depends on T034, T010b, T032)
- [X] T036 [US2] Implement `GET /sso/callback` in `server/app/routers/sso.py` at the provider's configured redirect path: call `await client.authorize_access_token(request)` (exchanges the code and verifies the ID token's signature/issuer/audience/nonce via Authlib), resolve a `UserIdentity` from its claims, evaluate mappings via `evaluate_access` (T025); on grant write `{email, role, provider_id}` to `request.session` and redirect (302) to the SPA's single landing route (FR-011); on denial, missing-attribute, or any Authlib exception (including consent denial at the provider) redirect (302) to `/login?error=<message>` without creating a session (FR-010) (depends on T035, T025, T032)
- [X] T037 [US2] Implement `GET /sso/session` (reads `request.session`, returns the same granted/role/email shape as `POST /sso/login`'s response) and `POST /sso/logout` (clears `request.session`, idempotent) in `server/app/routers/sso.py` (depends on T034, T032)
- [X] T038 [US2] Add `authorizeUrl()`, `getSession()`, and `logout()` to `client/src/api/sso.tsx`, each `fetch`ing with `credentials: 'include'` so the session cookie flows cross-origin (depends on T008)
- [X] T039 [US2] Add a "Sign in with `<Provider>`" link in `client/src/routes/login.tsx` pointing at `authorizeUrl()` (a plain navigation, not a `fetch` call), read an `?error=` search param to show the callback's denial message (FR-010), and relabel the existing email field as the demo-mode-only fallback (depends on T038)
- [X] T040 [US2] Have `AuthProvider` in `client/src/auth/AuthProvider.tsx` call `getSession()` on mount to restore auth state from the server-verified session cookie instead of trusting `localStorage` alone (`localStorage` becomes a paint-avoidance hint only), and have `logout()` also call the new session-clearing API (depends on T038)
- [X] T041 [P] [US2] Update `client/src/auth/AuthProvider.test.tsx` to cover session restore on mount, mocking `getSession` (depends on T040)
...
### Implementation for User Story 3 (continued — mapping precedence, closes deferred gap U1)

- [ ] T042c **[U1 fix] [US3]** Define and implement a deterministic precedence rule in `evaluate_access` (`server/app/services/access_evaluator.py`) for when two or more `AccessMapping` rules match the same identity — e.g., first-match-in-stored-order, or an explicit `priority` field on `AccessMapping` — and document the chosen rule in `contracts/sso-config-api.md`. Add a unit test in `server/tests/test_sso_mapping_evaluation.py` asserting the resolution when two mappings overlap (depends on T025)

### Implementation for User Story 3

- [X] T042b **[G3 fix] [US3]** Restrict `POST /sso/login`'s grant path in `server/app/routers/sso.py` to `demo_mode.enabled` only: remove the `provider is not None and provider.enabled` branch entirely so an enabled real provider can no longer be granted through a bare, unverified email POST — real-provider access MUST only be reachable via the verified `/sso/authorize` → `/sso/callback` redirect flow (T035/T036, FR-009/FR-010). When `demo_mode.enabled` is false, keep returning the existing "SSO provider is not configured or enabled" denial. Update `server/tests/test_sso_login_api.py` to match: `test_login_with_matching_mapping_is_granted_the_mapped_role`, `test_login_with_no_matching_mapping_is_denied`, `test_login_matching_a_blocked_mapping_is_denied`, and `test_login_missing_required_attribute_is_denied` currently configure only an enabled provider (no demo mode) and expect a grant/evaluated result — they must additionally enable demo mode to keep exercising mapping evaluation, and a new test must assert that an enabled provider **without** demo mode enabled now gets the "not configured or enabled" denial. **Documented, intentional API/behavior change** to an already-shipped endpoint (T026) per the constitution's Development Workflow compatibility clause — update the `POST /api/sso/login` entry in `contracts/sso-config-api.md` and `server/README.md` (T049) to state plainly that this endpoint never grants for a real provider, only for demo mode (depends on T026)
- [ ] T043 [US3] Enforce the `enabled` flag on `SSOProviderConfig` in `POST /sso/login`, returning a clear "provider unavailable" denial when disabled (FR-004) (depends on T026, T042b — after T042b, `POST /sso/login`'s only grant path is demo mode, so this enforcement now applies to `demo_mode.enabled`; real-provider `enabled` gating for the production flow is covered separately by T035's 503 check)
